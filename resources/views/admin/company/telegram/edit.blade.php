                                ...
                                <x-form.label for="{{ $employee->user->first_name }}_{{ $employee->user->last_name }}" class="w-full">
                                    <div class="flex justify-between w-full items-center">
                                        ...
                                        <x-form.checkbox 
                                            ...
                                            :checked="old('', $telegram_bot->settings->employees->{$employee->id} ?? null)"/>
                                    </div>
                                </x-form.label>
                            </div>
                        @endforeach
                            ...
                            <div class="flex space-x-2 items-center py-3 @if(!$loop->last) border-b @endif">
                                <x-form.label for="{{ $service->slug }}" class="w-full ">
                                    <div class="flex justify-between w-full items-center">
                                        ...
                                        <x-form.checkbox id="{{ $service->slug }}" name="settings[services][{{$service->id}}]" type="checkbox" :checked="old('settings[services][{{$service->id}}]', $telegram_bot->settings->services->{$service->id} ?? null)"/>
                                    </div>
                                </x-form.label>
                            </div>
                        @endforeach
