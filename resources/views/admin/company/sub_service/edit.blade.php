                                    ...
                                    <div class="flex space-x-2 items-center py-3 @if(!$loop->last) border-b @endif">
                                        <x-form.label for="{{ $employee->slug }}" class="w-full ">
                                            <div class="flex justify-between w-full items-center">
                                                ...
                                                <x-form.checkbox id="{{ $employee->slug }}" name="employees[]" :value="$employee->id" type="checkbox" :checked="old('employees[{{$employee->id}}]', !$sub_service->employees->where('id', $employee->id)->isEmpty())"/>
                                            </div>
                                        </x-form.label>
                                    </div>
                                @endforeach
                            ...
                            <div class="flex space-x-2 items-center py-3">
                                <x-form.label for="is_available_on_telegram" class="w-full">
                                    <div class="flex justify-between w-full items-center">
                                        ...
                                        <x-form.checkbox id="is_available_on_telegram" name="settings[is_available_on_telegram]" type="checkbox" :checked="old('settings[is_available_on_telegram]', $sub_service->settings->is_available_on_telegram ?? null)"/>
                                    </div>
                                </x-form.label>
                            </div>
                            <hr>
                            <div class="flex space-x-2 items-center py-3">
                                <x-form.label for="is_available_on_webapp" class="w-full">
                                    <div class="flex justify-between w-full items-center">
                                        ...
                                        <x-form.checkbox id="is_available_on_webapp" name="settings[is_available_on_webapp]" type="checkbox" :checked="old('settings[is_available_on_webapp]', $sub_service->settings->is_available_on_webapp ?? null)"/>
                                    </div>
                                </x-form.label>
                            </div>
                        </div>
                ...
                <x-white-block>
                    <div class="space-y-4">
                        ...
                        <x-form.select id="active" name="active" class="w-full">
                            <option @selected($sub_service->active == 1) value="1">Active</option>
                            <option @selected($sub_service->active == 0) value="0">Disable</option>
                        </x-form.select>
                    </div>
                </x-white-block>
