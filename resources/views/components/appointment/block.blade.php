...
<button wire:key="{{ $appointment->id }}" {{ $attributes->merge([
    'type' => 'button', 
    'class' => "border border-l-4 px-3 rounded-md w-full shadow-xl flex items-center space-x-3 hover:bg-gray-100 transform transition-all duration-200 $active"
    ...
    @if ($appointment->type == 'schedule')
        <x-a-buttons.edit class="my-1" wire:click="editSchedule({{ $appointment->id }})">
            Edit
        </x-a-buttons.edit>
    @endif
</button>