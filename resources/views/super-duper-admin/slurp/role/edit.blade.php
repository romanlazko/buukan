...
<x-app-layout>
    ...
    <div class="py-4 sm:p-4">
        <div class="w-full space-y-6 m-auto max-w-2xl">
            <x-white-block>
                <x-form.post method="patch" action="{{ route('super-duper-admin.role.update', $role) }}">
                    <div>
                    ...
                    <div>
                        ...
                        <x-form.input id="guard_name" name="guard_name" type="text" class="mt-1 block w-full" :value="old('guard_name', $role->guard_name)" required autocomplete="guard_name" />
                        <x-form.error class="mt-2" :messages="$errors->get('guard_name')" />
                    </div>

                    @forelse ($permissions->where('guard_name', $role->guard_name) as $permission)
                        <div>
                            <x-form.label for="{{ $permission->id }}">
                                <div class="w-full flex items-center space-x-2">
                                    <div class="flex-col">
                                        <x-form.input id="{{ $permission->id }}" name="permissions[]" type="checkbox" value="{{ $permission->id }}" :checked="($role->hasPermissionTo($permission->id))"/>
                                    </div>
